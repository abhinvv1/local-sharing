...
class LocalSharing extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.useSSL = options.useSSL !== false; // Default to true for security
    this.password = options.password || '';
    this.autoDiscover = options.autoDiscover !== false;
    this.trustAllCertificates = options.trustAllCertificates || false;
    
    // Key management options
    this.generateKeys = options.generateKeys !== false; // Default to true
    ...
    this.trustedCertificates = new Map(); // Store trusted certificates by deviceId
    ...
    if (this.useSSL) {
      ...
      const caPath = options.caPath; // Optional CA bundle
  ...
  _generateSelfSignedCertificate(privateKey) {
    // Node.js doesn't have built-in certificate generation
    // We'll use a simplified approach by using OpenSSL via child_process
    // For production use, consider using a proper library like node-forge
    ...
    try {
      ...
      const tempDir = path.join(os.tmpdir(), 'localsharing-cert-' + Date.now());
      if (!fs.existsSync(tempDir)) {
        fs.mkdirSync(tempDir, { recursive: true });
      }
      
      // Write the private key to a temp file
      const tempKeyPath = path.join(tempDir, 'private.key');
...
DNS.2 = localhost
      ...
      const opensslCmd = `openssl req -new -x509 -key ${tempKeyPath} -out ${tempCertPath} -days 730 -config ${configPath}`;
      ...
      // Fallback to a very basic self-signed cert
      // Note: This is only for testing and shouldn't be used in production
      console.log('Using fallback certificate generation method (less secure)');
  ...
  _extractPublicKeyFromCertificate(certificatePem) {
    const cert = crypto.createX509Certificate(certificatePem);
  ...
  trustDevice(deviceId, certificatePem) {
    try {
      ...
      this.trustedCertificates.set(deviceId, {
        ...
        fingerprint: this._calculateCertificateFingerprint(certificatePem)
      });
      
      this.emit('deviceTrusted', { deviceId });
      return true;
    } catch (error) {
  ...
  async sendMessage(deviceId, message) {
    ...
    const signature = this._signMessage(messageData);
    
    // Encrypt the message if we have the device's public key
    let encryptedData;
    if (device.publicKey) {
      // For simplicity, we're encrypting directly with public key
      // For larger messages, use hybrid encryption with a session key
      encryptedData = this._encryptMessageWithPublicKey(messageData, device.publicKey);
    } else {
      throw new Error(`No public key available for device ${deviceId}`);
    }
   ...
   * @private
   */
  _encryptMessageWithPublicKey(message, publicKey) {
    // For large messages, you would use hybrid encryption
    // (symmetric encryption for the message, asymmetric for the symmetric key)
  ...
  async _fetchDeviceCertificate(deviceId, address, port) {
    ...
    try {
      ...
      if (device) {
        // Extract public key from certificate
        const cert = crypto.createX509Certificate(response.certificate);
        ...
        device.certificateFingerprint = response.fingerprint;
        
        // Store in trusted certificates if fingerprint matches
        if (device.certFingerprint === response.fingerprint) {
          this.trustedCertificates.set(deviceId, {
  ...
  _handleMessageRoute(req, res) {
    ...
    const signature = req.headers['x-signature'] ? 
      Buffer.from(req.headers['x-signature'], 'base64') : null;
    ...
    req.on('end', async () => {
      try {
        ...
        const decryptedMessage = await this._decryptAndVerifyMessage(encryptedData, signature, certificate, sourceDeviceId);
  ...
  async _decryptAndVerifyMessage(encryptedData, signature, certificate, sourceDeviceId) {
    ...
    if (certificate) {
      ...
      const storedCert = this.trustedCertificates.get(sourceDeviceId);
      publicKey = crypto.createPublicKey(storedCert.publicKey);
      trusted = true;
    } else {
    ...
    try {
      const decipher = crypto.createDecipheriv('aes-256-cbc', symmetricKey.subarray(0, 32), iv);
...
// Import node-forge for certificate handling
let forge;
try {
  ...
  const { execSync } = require('child_process');
  try {
    execSync('npm install node-forge@^1.3.1');
    forge = require('node-forge');
  } catch (e) {
    console.error('Failed to install node-forge. Please install it manually: npm install node-forge@^1.3.1');
    process.exit(1);
  }
}
...
class LocalSharing extends EventEmitter {
  ...
  _extractPublicKeyFromCertificate(certificatePem) {
    try {
      // Try using native Node.js method if available (Node 15.6.0+)
      if (crypto.createX509Certificate) {
        const cert = crypto.createX509Certificate(certificatePem);
        return cert.publicKey.export({
          type: 'spki',
          format: 'pem'
